        ...
        <div class="table">
            ...
            >
                ...
                <el-table-column label="操作" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" @click="cellDBClick(scope.row)" style="font-size:14px;margin-right:10px">详情</el-button>
                        <el-button type="text" @click="generateHireBill(scope.row)" style="font-size:14px;margin-right:10px">生成拍租公告</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            showAddDialog() {
                if (this.flowFlag) {
                    ...
                    if (this.generateType === 'dialog') {
                        this.$refs.addSimplifyForm.showAddDialog(this.$history.currentRoute.query.id)
            ...
            generateHireBill(row) {
                ...
                this.$axios.post('cloud/download/generateHireBill', params, { responseType: 'blob' }).then(res => {
                    load.close()

                    if (res.data.type === 'application/octet-stream') {
                        let fileName = localStorage.getItem('alias') + '发布第' + row.year + row.period + '期房产拍租公告.docx'
                        let content = res.data
                        let blob = new Blob([content])
                        if ('download' in document.createElement('a')) {
                            // 非IE下载
