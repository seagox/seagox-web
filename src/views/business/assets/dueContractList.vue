        ...
        <div class="table">
            ...
            >
                ...
                <el-table-column label="操作" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" @click="relet(scope.row)" style="font-size:14px;margin-right:10px">续租</el-button>
                        <el-button type="text" @click="openSurrenderDialog(scope.row)" style="font-size:14px;margin-right:10px">终止</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            async queryById(type) {
                ...
                if (search['days'] > 90) {
                    this.$message.warning("剩余天数不能超过90天")
                    return
                }
                const params = {
            ...
            // 续租
            relet(row) {
                if (row.publish_again == 2) {
                    this.$message.warning("该资产已提前发布出租，不可续租")
                    return
                }
                var addFormId = 20 // 合同续租
                this.$history.push({
                    path: '/runCode',
                    query: {
                        id: addFormId,
                        path: 'reletContractForm',
                        title: '合同续租',
                        originalId: row.id,
                        reletId: row.relet_id ? row.relet_id : row.id,
                        time: new Date().getTime()
                    }
                })
            },

            openSurrenderDialog(row) {
                this.surrenderForm = {
                    id: row.id,
                    surrender_date: null,
                    surrender_type: null,
            ...
            surrenderSubmit() {
                this.$refs.surrenderForm.validate(valid => {
                    if (valid) {
                        let params = {
                            contract_id: this.surrenderForm.id,
                        ...
                        this.$axios.post('/cloud/entrance/contractSurrender', params).then(res => {
