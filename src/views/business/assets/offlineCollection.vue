                ...
                <el-tab-pane label="费用明细">
                    ...
                    <el-form :model="otherCostForm" ref="otherCostForm" size="small">
                        <el-table
                            ...
                            <el-table-column prop="period" align="center" width="240" label="期数"></el-table-column>
                            <el-table-column prop="month" align="center" width="240" label="月份"></el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            queryRentDetail(contract) {
                ...
                that.$axios.post('/cloud/entrance/queryUncollected', params).then(res => {
                    if (res.data.code == 200) {
                        that.feeDetailList = res.data.data
                        that.feeDetailList.map(val => {
                            let type
                            if (val.name == 5) {
                                type = val.typeName + ' - 期数' + val.period + ' - 金额' + val.amount
                            } else if (val.name == 6 || val.name == 7) {
                                type = val.typeName + ' - ' + val.amount
                            } else {
                                // 应收单
                                type = val.typeName + ' - 账单月份' + val.month + ' - 金额' + val.amount
                            }
                            val.type = type
                            val.choose = false
                        })
            ...
            handleAddFee() {
                ...
                let type
                if (canChoose.name == 5) {
                    type = canChoose.typeName + ' - 期数' + canChoose.period + ' - 金额' + canChoose.amount
                } else if (canChoose.name == 6 || canChoose.name == 7) {
                    type = canChoose.typeName + ' - ' + canChoose.amount
                } else {
                    // 应收单
                    type = canChoose.typeName + ' - 账单月份' + canChoose.month + ' - 金额' + canChoose.amount
                }
                // 赋值
                this.otherCostForm.otherCostData.push({
                    ...
                    name: canChoose.name,
                    receipt_date: this.formModel['assets_offline_collection.payment_time'],
                    contract: this.contract,
                    period: canChoose.period,
                    month: canChoose.month,
                })
                this.changeChoose = true
            },
            // 删除费用明细
            ...
            typeChange(val, index) {
                ...
                this.feeDetailList.forEach(item => {
                    if (item.id === val) {
                        this.$set(this.otherCostForm.otherCostData[index], 'id', item.id)
                        ...
                        this.$set(this.otherCostForm.otherCostData[index], 'period', item.period)
                        this.$set(this.otherCostForm.otherCostData[index], 'month', item.month)
                    }
                })
                // 遍历是否已经被选择
                this.selectChangeJudge()
            ...
            async queryHtInfo() {
                ...
                this.loading = true
                let res = await this.$axios.get('cloud/authority/queryCollectibleContract', {params})
                this.loading = false
                if (res.data.code == 200) {
            ...
            ysCellDBClick(row, column, cell, event) {
                ...
                this.$set(this.formModel, 'assets_offline_collection.contract', row.id)
                this.$set(this.formModel, 'assets_offline_collection.assets_id', row.assetsId)
                this.$set(this.formModel, 'assets_offline_collection.assets_name', row.assetsName)
                this.$set(this.formModel, 'assets_offline_collection.tenement', row.tenement)
                this.queryByMark()
