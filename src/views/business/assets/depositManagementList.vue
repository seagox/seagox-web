<template>
    <div v-loading="loading">
        <div class="toolView">
            ...
            <el-button type="text" icon="el-icon-refresh-left" @click="goRefundAdd" size="small">退款</el-button>
        </div>
        ...
        <div class="table">
            ...
            >
                ...
                <el-table-column label="操作" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" @click="eidtRefund(scope.row)" :disabled="!scope.row.refund_id" style="font-size:14px;margin-right:10px">编辑退款单</el-button>
        ...
        methods: {
                ...
                if (res.data.code == 200) {
                    ...
                    if (type === 1) {
                        ...
                        for (let k = 0; k < this.searchJsonList.length; k++) {
                            if (this.searchJsonList[k].field === 'days') {
                                this.searchJsonList[k].value = 30
                            }
            ...
            cellDBClick(row, column, cell, event) {
                if (!this.routerScript) {
                    // 退回走编辑页面
                    if (row["info_status"] == 2 && row["user_id"] == localStorage.getItem('userId')) {
                        this.$history.push({
                            path: '/editCustomForm',
                            query: {
                                formId: this.formId,
                        ...
                        this.$history.push({
                            ...
                            query: {
                                ...
                                formId: this.formId,
                                id: row.id,
                                title: this.$history.currentRoute.query.title + '详情'
                            }
                        })
                    }
                } else {
            ...
            goRefundAdd() {
                ...
                var addFormId = 22// 押金退款单
                this.$history.push({
                    path: '/runCode',
                    query: {
                        id: addFormId,
                        path: 'contractRefundAddForm',
                        title: '租赁资产费用退款单',
                        receiptId: this.multipleSelection[0].id,
                        userName: this.multipleSelection[0].payer,
                        receiptCompany: this.multipleSelection[0].receipt_company,
                        receiptAccount: this.multipleSelection[0].receipt_account,
                        receiptBank: this.multipleSelection[0].receipt_bank,
                        time: new Date().getTime()
                    }
                })
            }
