                            ...
                            <el-table-column prop="type" align="center" label="申请退款费用类型">
                                <template slot-scope="scope">
                                    <el-form-item :prop="'otherCostData.' + scope.$index + '.type'">
                                        <el-select v-model="scope.row.label" @change="typeChange($event, scope.$index)" filterable placeholder="请选择费用类型">
                                            <el-option
    ...
    export default {
        ...
        methods: {
            ...
            handleAddFee() {
                ...
                let label
                if (canChoose.type == 5) {
                    label = canChoose.sourceName + ' - ' + canChoose.typeName + ' - 期数' + canChoose.period + ' - 金额' + canChoose.amount
                } else if (canChoose.type == 6 || canChoose.type == 7) {
                    label = canChoose.sourceName + ' - ' + canChoose.typeName + ' - ' + canChoose.amount
                } else {
                    // 应收单
                    label = canChoose.sourceName + ' - ' + canChoose.typeName + ' - 金额' + canChoose.amount
                }
            ...
            async queryHtInfo() {
                ...
                this.loading = true
                let res = await this.$axios.get('cloud/authority/queryRefundContract', {params})
                this.loading = false
                if (res.data.code == 200) {
            ...
            ysCellDBClick(row, column, cell, event) {
                ...
                this.$set(this.formModel, 'assets_refund_application.contract', row.id)
                this.$set(this.formModel, 'assets_refund_application.assets_id', row.assetsId)
                this.$set(this.formModel, 'assets_refund_application.assets_name', row.assetsName)
                this.queryByMark()
