    ...
    <div class="container" v-loading="loading">
        ...
        <div style="margin-bottom: 20px">
            <el-tabs type="border-card">
                <el-tab-pane label="房租明细">
                    <el-button type="primary" size="small" icon="el-icon-plus" style="margin-bottom: 5px" @click="handleAddHouseBill">新增</el-button>
                    <el-form :model="houseBillTableForm" ref="houseBillTableForm" size="small">
                        <el-table :data="houseBillTableForm.houseBillData" style="width: 100%">
                            <el-table-column prop="type" align="center" width="240" label="期数">
                                <template slot-scope="scope">
                                    <el-form-item :prop="'houseBillData.' + scope.$index + '.period'">
                                        <el-select v-model="scope.row.period" @change="periodChange($event, scope.$index)" placeholder="请选择期数">
                                            <el-option
                                                v-for="(item, index) in rentDetailList"
                                                :key="index"
                                                :label="item.period"
                                                :value="item.period">
                                            </el-option>
                            ...
                            <el-table-column prop="start_date" align="center" label="开始日期">
                                <template slot-scope="scope">
                                    <el-form-item :prop="'houseBillData.' + scope.$index + '.start_date'">
                                        <el-date-picker
                            ...
                            <el-table-column prop="end_date" align="center" label="结束日期">
                                <template slot-scope="scope">
                                    <el-form-item :prop="'houseBillData.' + scope.$index + '.end_date'">
                                        <el-date-picker
                            ...
                            <el-table-column prop="amount" align="center" width="240" label="收款金额">
                                <template slot-scope="scope">
                                    <el-form-item :prop="'houseBillData.' + scope.$index + '.amount'">
    ...
    export default {
        ...
        methods: {
            ...
            queryRentDetail(contract) {
                ...
                that.$axios.post('/cloud/entrance/queryRentDetail', params).then(res => {
                    if (res.data.code == 200) {
                        that.rentDetailList = res.data.data
                        if (that.rentDetailList.length == 0) {
                            that.$message.warning('该合同不存在未收房租！')
                        }
                    } else {
                        that.$message.error('房租明细查询失败！')
                    }
                })
            },
            // 新增房租明细
            handleAddHouseBill() {
                if (this.rentDetailList.length == 0) {
                    this.$message.warning('该合同不存在未收房租！')
                    return
                }
                this.houseBillTableForm.houseBillData.push({
                    period: this.rentDetailList[0].period,
                    start_date: this.rentDetailList[0].start_date,
                    end_date: this.rentDetailList[0].end_date,
                    amount: this.rentDetailList[0].amount,
                    receipts_info_id: this.rentDetailList[0].receipts_info_id,
                    name: 5,
            ...
            periodChange(val, index) {
                this.rentDetailList.forEach(item => {
                    if (item.period === val) {
                        this.$set(this.houseBillTableForm.houseBillData[index], 'period', item.period)
                        this.$set(this.houseBillTableForm.houseBillData[index], 'start_date', item.start_date)
                        this.$set(this.houseBillTableForm.houseBillData[index], 'end_date', item.end_date)
                        this.$set(this.houseBillTableForm.houseBillData[index], 'amount', item.amount)
                    }
                })
            },
            // =================================================新增应收单==============================
