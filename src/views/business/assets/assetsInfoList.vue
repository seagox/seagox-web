<template>
    <div v-loading="loading">
        <div class="toolView">
            ...
            <el-button type="text" icon="el-icon-user" @click="showMemberDialog" size="small" v-if="$history.currentRoute.query.id == 6">变更管理人员</el-button>
            <el-button type="text" icon="el-icon-edit" @click="goModifyForm" size="small" v-if="$history.currentRoute.query.id == 6">变更</el-button>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            showMemberDialog() {
                if (this.multipleSelection.length === 0) {
                    this.$message.warning('亲，请先选择要变更管理人员的数据')
                    return
                }
                this.$refs.excelMember.showDialog('default', null)
            },
            ...
            memberConfirm(type, field, value) {
                ...
                this.loading = true
                this.$axios.get('cloud/entrance/batchUpdateAssetsInfoManageId', { params }).then(res => {
                    this.loading = false
                    if (res.data.code === 200) {
                        this.$message.success("变更管理人员成功")
            ...
            goModifyForm() {
                ...
                // 检查是否可以变更
                let params = {
                    assets_id: this.multipleSelection[0]['id']
                }
                this.loading = true
                    ...
                    if (res.data.code === 200) {
                        this.$history.push({
                            ...
                            query: {
                                ...
                                id: 34, // 资产变更
                                originAssetsId: this.multipleSelection[0]['id'],
