    ...
    <div class="searchView disposeNew">
      <el-form :model="searchForm" ref="searchForm" label-width="110px" :rules="rules" :inline="true">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item>
              <el-button type="primary" icon="el-icon-folder-add" @click.native="edit">保存编辑</el-button>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    queryByPage() {
      ...
      that.$axios.post('cloud/entrance/getDisposeByDjId', params).then(res => {
        if (res.data.code == 200) {
          console.log(res.data)
          that.tableData = res.data.data.tableData
          that.searchForm = res.data.data.searchForm
        }
      })
    },
    ...
    //判断是否有提计
    edit() {
      if (this.$history.history.current.query.id==null){
        return  this.$message.error("无单据id")
      }
      let that = this
      const params = {
        djId: this.$history.history.current.query.id,
      }
      that.$axios.post('cloud/entrance/ruleEdit', params).then(res => {
        if (res.data.code == 200) {
          that.add()
        } else {
        ...
        if (res.data.code == 200) {
          this.$refs.searchForm.validate(valid => {
            if (valid) {
              ...
              that.$axios.post('cloud/entrance/editAdd', params).then(res => {
                if (res.data.code == 200) {
