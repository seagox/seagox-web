<template>
  <div>
    <div class="searchView splitDetails">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="本月计提">
              <el-checkbox disabled v-model="searchForm.checkedByjt">{{ '' }}</el-checkbox>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item>
              <el-button type="primary" icon="el-icon-edit" @click.native="add">保存编辑</el-button>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <div class="table">
      ...
      <el-table max-height="450px" :summary-method="getSummaries" show-summary @cell-dblclick="editCell"
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+1" label="资产编码" prop="assets_code" align="center">
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+2" label="资产名称" prop="assets_name" align="center">
          <template slot-scope="{row,column}">
            <el-input
                @blur="alterData(row,column)"
                :ref="column.property"
        ...
        <el-table-column :key="randomKey+3" label="规格型号" prop="model" align="center">
          <template slot-scope="{row,column}">
            <el-input
                @blur="alterData(row,column)"
                :ref="column.property"
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+5" label="数量" prop="number" align="center">
          <template slot-scope="{row,column}">
            <el-input
                @blur="alterData(row,column)"
                :ref="column.property"
        ...
        <el-table-column :key="randomKey+6" label="计量单位" prop="jldw" align="center">
          <template slot-scope="{row,column}">
            <el-input
                @blur="alterData(row,column)"
                :ref="column.property"
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+7" label="原值" prop="original_value" align="center">
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+8" label="进项税额" prop="jxse" align="center">
          <template slot-scope="{row,column,$index}">
            <el-input
        ...
        <el-table-column :render-header="addRedStar" :key="randomKey+9" label="累计折旧/摊销" prop="ljzj_tx" align="center">
          <template slot-scope="{row,column,$index}">
            <el-input
...
export default {
  ...
  methods: {
    ...
    queryByPage() {
      ...
      that.$axios.post('cloud/entrance/getSplitMxByDJ', params).then(res => {
        if (res.data.code == 200) {
          that.tableData = JSON.parse(res.data.data.cfhmx)
          that.searchForm = JSON.parse(res.data.data.cfqmx)
        }
      })
    },
    //查询资产属性列表
        ...
        this.$refs.searchForm.validate(valid => {
          if (valid) {
            ...
            const params = {
              ...
              djId: this.$history.history.current.query.id
            }
            that.$axios.post('cloud/entrance/splitEdit', params).then(res => {
              if (res.data.code == 200) {
      ...
      if (this.searchForm.checkedAslc == true) {
        ...
        for (let i = 0; i < number; i++) {
          let obj = {
            ...
            original_value: original_value / number,
            jxse: jxse / number,
            ljzj_tx: ljzj_tx / number,
            net_value: net_value / number,
            card_date: this.searchForm.card_date,
            isShow: true
          };
...
.splitDetails .el-input.is-disabled .el-input__inner{
