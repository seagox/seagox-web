<template>
    <div class="container">
            ...
            <el-main>
                ...
                <div style="border-bottom: 2px solid #e5e9f2; margin: 10px 20px">
                    <el-tabs v-model="status" @tab-click="handleClick">
                        <el-tab-pane label="未整改" name="2"></el-tab-pane>
                        <el-tab-pane label="已整改" name="3"></el-tab-pane>
                        <el-tab-pane label="被退回" name="4"> </el-tab-pane>
                        <el-tab-pane label="曾经办" name="5"> </el-tab-pane>
                    </el-tabs>
                </div>
                <div class="toolView" style="padding: 0 10px">
                    <el-button v-if="status == 2 || status == 4" size="mini" type="primary" @click="zgShow()"
                        >整改</el-button
                    >
                    <el-button v-if="status == 3" size="mini" type="primary" @click="updateStatus(2)">撤销</el-button>
                ...
                <div class="table">
                    ...
                    <el-table :data="tableData" max-height="500" border stripe highlight-current-row ref="multiple">
                        ...
                        <el-table-column align="center" label="整改说明">
                            <template slot-scope="scope">
                                <span class="el-dropdown-link">
                                    ...
                                    <br v-if="scope.row.zgFileName" />
                                </span>
                                <el-dropdown @command="handleCommandZg($event, scope.row)">
                                    <span class="el-dropdown-link">
                                        <span>{{ scope.row.zgFileName }}</span>
                                    </span>
                        ...
                        <el-table-column prop="xdTime" label="发表时间" width="160" align="center"></el-table-column>
                    </el-table>
                    <!--分页-->
        ...
        <el-dialog
            ...
            :close-on-click-modal="false"
        >
            <el-form ref="zgForm" label-width="120px" :rules="rules" :model="zgForm">
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item label="整改说明" prop="zgOpinion">
                            <el-input v-model="zgForm.zgOpinion" type="textarea" :rows="3"></el-input>
                        </el-form-item>
                    </el-col>

                    <el-col :span="18">
                        <el-form-item label="整改文件" prop="zgFilePath">
                            <el-input v-model="zgForm.zgFileName" disabled></el-input>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button size="small" @click="zgDialog = false" type="default">取 消</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            zgDialog: false,
            rules: {
                zgOpinion: [{ required: true, message: '请输入整改说明', trigger: 'bulr' }],
                zgFilePath: [{ required: true, message: '请选择上传整改报告文件', trigger: 'change' }]
            }
        }
    },
    created() {
    ...
    methods: {
        ...
        handleCommandZg(command, row) {
            let url = row.zgFilePath
            let fileName = row.zgFileName
        ...
        zgShow() {
            ...
            if (_selectData.length != 1) {
                return that.$message.warning('请选择一条数据进行整改!')
            }
            this.zgForm.status = '3'
            this.zgForm.id = _selectData[0].id
            this.zgForm.zgOpinion = _selectData[0].zgOpinion || ''
            this.zgForm.zgFilePath = _selectData[0].zgFilePath || ''
            this.zgForm.zgFileName = _selectData[0].zgFileName || ''
            this.zgForm.accountId = this.userId
            this.zgForm.roleName = this.userName
            this.zgForm.remark = '整改'
            this.zgDialog = true
        },
        //文件上传前判断
        beforeUpload(file) {
        ...
        successHandle(res, file, fileList) {
            if (res.code === 200) {
                ...
                this.zgForm.zgFileName = file.name
                this.zgForm.zgFilePath = res.data
            } else {
        ...
        submitZg() {
            let that = this
            this.$refs.zgForm.validate(valid => {
                if (valid) {
                    that.zgForm.ids = that.zgForm.id
                    that.$axios.post('cloud/entrance/updateScDhReportStatus', that.zgForm).then(res => {
                        if (res.data.code == 200) {
                            that.$message.success('提交整改成功!')
                            that.zgDialog = false
                            that.pageNo = 1
                            that.queryByPage()
                        } else {
        ...
        updateStatus(status) {
            ...
            this.$confirm('确定要撤销所选数据吗？', '提示', { type: 'warning' })
                .then(() => {
                    let idss = []
