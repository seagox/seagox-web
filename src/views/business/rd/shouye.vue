            ...
            <div style="width: 66%">
                <div style="margin: 10px; position: relative">
                    <el-card :style="'height:350px'">
                        ...
                        <div class="table-box" :style="'height:220px'" v-if="secondPart">
                            <div>
                                <el-table
                                    ref="table"
                                    ...
                                    stripe
                                    height="220"
                                    :row-style="{ background: curColor + '15' }"
                        ...
                        <div style="margin-top: 10px ">
                            截止日期
                            <span :style="{ margin: '0 10px', color: curColor }">{{ secondPart.jzrq }}</span>
                            累计执行
                            <span :style="{ margin: '0 10px', color: curColor }">{{ secondPart.ljzxNum }}</span
                            >笔/ <span :style="{ margin: '0 10px', color: curColor }">{{ secondPart.ljzxMoney }}</span
                            >万元
                        </div>
                    </el-card>
                </div>
                        ...
                        <div class="tabs-box">
                            <div>
                                <div
                                    ...
                                    :key="indexs"
                                    @click="thirdValue = items.configName"
                                    style="display:inline-block"
                                >
                                    <div
                                        class="cus-tab"
                                        :style="{
                                            background: thirdValue == items.configName ? curColor : '',
                                            color: thirdValue == items.configName ? '#fff' : ''
                                        }"
                                    >
                                        {{ items.configName }}
                                    </div>
                                    <div
                                        class="is-active-arrow"
                                        v-if="thirdValue == items.configName"
                                        :style="{
                                            'border-top': '8px solid ' + curColor
                                        }"
                                    ></div>
                                </div>
                            </div>
                            <div
                                v-for="(items, indexs) in thirdPart"
                                :key="indexs"
                                v-show="thirdValue == items.configName"
                            >
                                    ...
                                    <div class="tab-pane-item ">
                                        <div class="flex-between">
                                            ...
                                            ><span
                                                ...
                                                @click="
                                                    pageJump('/runCode?path=scOpinionInquiryCenter&title=审查意见查询')
                                                "
                                                >详情</span
                                            >
                                        </div>
                                        ...
                                        <div :id="'tubiao-2-' + indexs" style="width: 100%; height: 200px"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tabs-box" :style="'height:540px'">
                        <div>
                            <div
                                ...
                                :key="indexs"
                                @click="fourthValue = items.title"
                                style="display:inline-block"
                            >
                                <div
                                    class="cus-tab"
                                    :style="{
                                        background: fourthValue == items.title ? curColor : '',
                                        color: fourthValue == items.title ? '#fff' : ''
                                    }"
                                >
                                    {{ items.title }}
                                </div>
                        ...
                        <div v-for="(items, indexs) in fourthPart" :key="indexs" v-show="fourthValue == items.title">
                            <div class="">
                                ...
                                <el-table :data="items.list" :show-header="false">
                                    ...
                                    <el-table-column width="50">
                                        <template slot-scope="slot">
                                            <span style="color: #da001c; font-size: 14px">•</span>{{ slot.row.yzNum }}
                                        </template>
                                    </el-table-column>
                                    <el-table-column width="50">
                                        <template slot-scope="slot">
                                            <span style="color: #fb5f14; font-size: 14px">•</span>{{ slot.row.jgNum }}
                                        </template>
                                    </el-table-column>
                                    <el-table-column width="50">
                                        <template slot-scope="slot">
                                            <span style="color: #ffc034; font-size: 14px">•</span>{{ slot.row.tsNum }}
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </div>
                        ...
                        <div v-if="fivePart" class="sub-list-box">
                            ...
                            <el-table :data="fivePart" v-else>
                                ...
                                <el-table-column prop="countNum" width="100" label="数量"></el-table-column>
                            </el-table>
                        </div>
                    </div>
                </el-card>
            ...
            this.$axios.get('/cloud/entrance/getIndexSrOrZcInfoByZjxz').then(res => {
                if (res.data.code == 200) {
                    ...
                    that.firstPart.forEach((key, index) => {
                        switch (index) {
                            case 0:
                                key.srUrl = '/reportList?id=71&title=全市一般公共预算收入完成情况表'
                                key.zcUrl = '/reportList?id=87&title=全市一般公共预算支出完成情况表'
                                break
                            case 1:
                                key.srUrl = '/reportList?id=75&title=全市政府性基金预算收入完成情况表'
                                key.zcUrl = '/reportList?id=76&title=全市政府性基金预算支出完成情况表'
                                break
                            case 2:
                                key.srUrl = '/reportList?id=79&title=全市国有资本经营预算收入完成情况表'
                                key.zcUrl = '/reportList?id=80&title=全市国有资本经营预算支出完成情况表'
                                break
                            case 3:
                                key.srUrl = '/reportList?id=83&title=全市社会保险基金预算收入完成情况表'
                                key.zcUrl = '/reportList?id=84&title=全市社会保险基金预算支出完成情况表'
                                break
                        }
                    })
                }
