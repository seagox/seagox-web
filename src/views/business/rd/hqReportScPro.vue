    ...
    <div class="container">
        <div class="table">
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="意见采编" name="first"></el-tab-pane>
                <el-tab-pane label="意见处理" name="second"></el-tab-pane>
                <el-tab-pane label="提交报告" name="third"></el-tab-pane>
                <el-tab-pane label="已送审" name="fourth"></el-tab-pane>
                <el-tab-pane label="被退回" name="fifth"></el-tab-pane>
                <el-tab-pane label="曾经办" name="sixth"></el-tab-pane>
            </el-tabs>
            <hr />
            <div class="searchView" v-if="activeName === 'first' || activeName === 'second'">
            ...
            >
                ...
                <el-table-column align="center" label="完成状态" prop="status">
                    <template slot-scope="scope">
                        <div :style="{ color: scope.row.status === 1 ? 'blue' : 'red' }">
            ...
            <div class="toolView">
                <el-button v-if="activeName === 'first'" size="small" type="text" @click="subOrRevOpinion(4)"
                    >采编
                </el-button>
                <el-button v-if="activeName === 'second'" size="small" type="text" @click="subOrRevOpinion(5)"
                    >剔除
                </el-button>
                <el-button v-if="activeName === 'second'" size="small" type="text" @click="scbgShow"
                    >生成报告</el-button
                >
                <el-button
                    v-if="activeName === 'third' || activeName === 'fifth'"
                    size="small"
                    type="text"
                    @click="bgOperation(0)"
                    >送审</el-button
                ...
                <el-button
                    ...
                    >删除</el-button
                >
                <el-button v-if="activeName === 'fourth'" size="small" type="text" @click="bgOperation(-2)"
                    >撤销</el-button
                >
                <el-button
            ...
            >
                ...
                <el-table-column align="center" label="条目类别" prop="type">
                    <template slot-scope="scope">
                        <div :style="{ color: 'red' }">
                            {{
            ...
            >
                ...
                <el-table-column align="center" label="预审报告" prop="reportName">
                    <template slot-scope="scope">
                        <el-dropdown @command="handleCommand($event, scope.row)">
                            <span class="el-dropdown-link">
                                <span>{{ scope.row.reportName }}</span>
            ...
            <!--分页-->
            <div class="pagination" v-if="!(activeName === 'first' || activeName === 'second')">
                <el-pagination
...
export default {
    ...
    methods: {
        ...
        queryXzReport() {
            ...
            that.$axios.get('cloud/authority/getXzReportListByXz', { params }).then(res => {
                if (res.data.code === 200) {
                    this.tableData1 = res.data.data
                    if (this.tableData1 && this.tableData1.length > 0) {
                        for (let i = 0; i < this.tableData1.length; i++) {
                            if (this.tableData1[i].status === 1) {
                                this.searchForm.xzReportId.push(this.tableData1[i].id)
                            }
                        }
                        if (this.searchForm.xzReportId.length === 0) {
                            this.searchForm.xzReportId.push(0)
                        }
                        this.queryPage()
                    } else {
                        this.tableData2 = []
        ...
        queryPage() {
            ...
            const params = {
                ...
                xzReportId: this.searchForm.xzReportId.toString(),
                flag: 'hqbgscylc'
            }
            if (this.searchForm.taskId) {
                params.taskId = this.searchForm.taskId
            ...
            if (this.activeName === 'second') {
                params.departmentCode = this.searchForm.departmentCode
            }
        ...
        queryReportPage() {
            ...
            that.$axios.get('cloud/authority/getHqReportListByStatus', { params }).then(res => {
                if (res.data.code === 200) {
        ...
        handle(row, event, column) {
            this.searchForm.xzReportId = []
            if (row.id && row.status === 1) {
                this.searchForm.xzReportId.push(row.id)
            } else {
                this.searchForm.xzReportId.push(0)
            }
            this.queryPage()
        ...
        scbgShow() {
            ...
            this.scbgForm.reportName =
                selectData[0].year + selectData[0].scTypeName + '-' + selectData[0].taskName + '总报告' + '.docx'
        ...
        getlogTableData() {
            let _selectData = this.$refs.multipleTable3.selection
