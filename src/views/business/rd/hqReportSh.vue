<template>
    <div class="container hqReportSh-page">
        <div class="table">
            <el-tabs v-model="activeName" @tab-click="handleClick" type="card">
                <el-tab-pane label="待审核" name="first">
                    <div class="toolView">
                        <el-button style="font-size:14px" size="small" round type="plain" @click="approve"
                            >审核</el-button
                        >
                        <el-button style="font-size:14px" size="small" round type="plain" @click="goback"
                            >退回</el-button
                        >
    ...
    methods: {
        ...
        goback() {
            ...
            this.$confirm(`确定要退回所选数据吗？`, '提示', { type: 'warning' }).then(() => {
                ...
                this.$axios.post('cloud/entrance/returnReport', param).then(res => {
        ...
        approve() {
            ...
            this.$confirm(`确定要审批所选数据吗？`, '提示', { type: 'warning' }).then(() => {
                let idss = []
                for (let i = 0; i < _selectData.length; i++) {
                    idss.push(_selectData[i].id)
                }
                ...
                this.$axios.post('cloud/entrance/approve', param).then(res => {
                    this.$message({
                        message: '审批成功',
                        type: 'success'
                    })
...
.hqReportSh-page .el-tabs__header .el-tabs__item.is-active {
    border-bottom: 1px solid #e4e7ed;
    background: #efefef;
}
.hqReportSh-page .toolView {
    margin-bottom: 5px;
    padding-left: 0;
}
.container {
