<template>
	<div>
		...
		>
			...
			<el-table :data="tableData" border highlight-current-row stripe>
				...
				<el-table-column label="操作" align="center" width="185">
					<template slot-scope="scope">
						<el-button type="text" @click="showEditDialog(scope.$index, scope.row)" size="small"
							>编辑</el-button
						>
		...
		<el-dialog title="新增" width="550px" append-to-body :visible.sync="addVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px" :rules="rules" ref="addForm">
				...
				<el-form-item label="是否合计" v-if="addForm.type === 'number'">
					<el-switch v-model="addForm.summary"></el-switch>
				</el-form-item>
				<el-form-item
					label="数据来源"
					prop="source"
					v-if="addForm.type === 'radio' || addForm.type === 'checkbox' || addForm.type === 'select'"
				>
				...
				<el-form-item label="业务类型" prop="businessType" v-if="addForm.type === 'business'">
					<el-select
				...
				<el-form-item label="显示字段" prop="showField" v-if="addForm.type === 'business'">
				...
				<el-form-item label="公式" prop="calculation" v-if="addForm.type === 'calculation'">
					<el-input
						type="textarea"
						:rows="3"
						v-model="addForm.calculation"
						placeholder="请输入公式"
					></el-input>
				</el-form-item>
			</el-form>
		...
		<el-dialog title="编辑" width="550px" append-to-body :visible.sync="editVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="rules" ref="editForm">
				...
				<el-form-item label="是否合计" v-if="editForm.type === 'number' || editForm.type === 'calculation'">
					<el-switch v-model="editForm.summary"></el-switch>
				</el-form-item>
				<el-form-item
					label="数据来源"
					prop="source"
					v-if="editForm.type === 'radio' || editForm.type === 'checkbox' || editForm.type === 'select'"
				>
				...
				<el-form-item label="业务类型" prop="businessType" v-if="editForm.type === 'business'">
					<el-select
				...
				<el-form-item label="显示字段" prop="showField" v-if="editForm.type === 'business'">
				...
				<el-form-item label="公式" prop="calculation" v-if="editForm.type === 'calculation'">
					<el-input
						type="textarea"
						:rows="3"
...
export default {
	...
	data() {
		return {
			...
			rules: {
				...
				calculation: [{ required: true, message: '请输入公式', trigger: 'blur' }],
	...
	methods: {
		...
		async queryBusinessField(tableName) {
			let res = await this.$axios.get('businessField/queryAll?display=1&tableName=' + tableName)
			if (res.data.code === 200) {
		...
		showDialog() {
			this.queryBusinessField(this.data.field)
		...
		handleClose() {
			...
			this.$set(this.data, 'values', this.tableData)
		}
	}
}
</script>
