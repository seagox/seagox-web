        ...
        <el-dialog title="新增" width="500px" :visible.sync="addSearchVisible" :close-on-click-modal="false">
            <el-form :model="addSearchForm" label-width="80px" :rules="searchRules" ref="addSearchForm">
                ...
                <el-form-item label="数据源" prop="source" filterable v-if="addSearchForm.type === 'cascader'">
                    <el-select v-model="addSearchForm.source" placeholder="请选择数据源" clearable filterable>
                        <el-option
...
export default {
        ...
        return {
            ...
            excelOption: {
                view: {
                    ...
                    width: () => {
                        return document.getElementsByClassName('container')[0].clientWidth - 30
                    }
                },
                showBottomBar: false,
                row: {
    ...
    methods: {
        async queryById() {
            let res = await this.$axios.get('jellyReport/queryById/' + this.$route.query.id)
            if (res.data.code === 200) {
                this.form = res.data.data
                if (this.form.exportPath) {
                    this.form.exportPath = JSON.parse(this.form.exportPath)
                } else {
                    this.form.exportPath = []
                }
                if (res.data.data.templateSource) {
        ...
        handleNext() {
            if (this.active === 0) {
                ...
                if (!this.xs) {
                    ...
                    // this.xs.sheet.overlayerEl.on('mousemove', evt => {
                    //     // overlayerMousemove.call(this, evt)
                    //     console.log('mousemove', evt)
                    // })
                    this.xs.on('cells-selected', (cell, { sri, sci, eri, eci }) => {
                        //sri左上y坐标, sci左上x坐标, eri右下y坐标, eci右下x坐标,都是格子序号
                        let leftTopX = document
                            .getElementsByClassName('x-spreadsheet-overlayer-content')[0]
                            .getBoundingClientRect().left
                        let lefttopY = document
                            .getElementsByClassName('x-spreadsheet-overlayer-content')[0]
                        ...
                        if (this.mouseX > rightBottomX) {
                            this.xs.sheet.horizontalScrollbar.move({
                                left:
                                    120 * (eci + 1) -
                                    Number(
                                        document
                        ...
                        if (this.mouseY < leftTopX) {
                            this.xs.sheet.verticalScrollbar.move({
                                top:
                                    32 * (sri + 1) <=
                                    Number(
                                        document
                        ...
                        } else if (this.mouseY > rightBottomY) {
                            this.xs.sheet.verticalScrollbar.move({
                                top:
                                    32 * (eri + 1) -
                                    Number(
        ...
        submitForm() {
            ...
            this.$refs['ruleForm'].validate(valid => {
                if (valid) {
                    ...
                    that.$axios.post('jellyReport/update', params).then(function (response) {
